...
All experiments, models, preprocessing, decoding, and visual analysis are implemented in a **two respective Jupyter notebooks**, which contain a **fully documented, block-by-block explanation of the entire codebase** (note that the CTC notebook is currently being updated and is therefore incomplete. The Seq2Seq Notebook, however is complete). You can also find a PPT folder that presents the full project and results.
